...
open_dev <- function(fname, picdev)
{
   if (picdev == 1)
   {
...
open_dev2 <- function(fname, picdev)
{
   if (picdev == 1)
   {
...
{
   plotfname = paste(path, func, "_plot_avg_", substr(aidarr_str,1,46), sep="") 
   histfname = paste(path, func, "_hist_avg_", substr(aidarr_str,1,46), sep="") 
   
   for (picdev in 1:2)
   {
      open_dev(plotfname, picdev)
      narr <- c(1:tot)  
...
write("##The end", outfname, 1, append=TRUE, sep="")  
